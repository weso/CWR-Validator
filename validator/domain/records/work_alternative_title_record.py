...
class WorkAlternativeTitleRecord(DetailHeader):
    ...
    def __init__(self, record, transaction):
        super(WorkAlternativeTitleRecord, self).__init__(record, transaction)
    ...
    def validate(self):
        if self.attr_dict['Title type'] not in TITLE_TYPES:
            ...
            self._rejected_fields['Title type'] = FieldRejectedError('Given title type not in table', self._record,
                                                                     'Title type', 'AT')
